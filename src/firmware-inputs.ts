...
async function pickFile(args?: PickFileArgs): Promise<string | undefined> {
  const uris = await vscode.window.showOpenDialog({
    ...
    defaultUri: resolveDefaultUri(args?.defaultUri),
  });
...
async function pickFolder(args?: PickFolderArgs): Promise<string | undefined> {
  const uris = await vscode.window.showOpenDialog({
    title: args?.title,
